...
<!--===========================  Chinese Edition Below  =================================-->
    ...
    <xsl:if test="/manpage/routine_idx[.$ge$' ']"><p style="font-size:90%;font-weight:bold;margin-left:0"><A class="method"><xsl:attribute name="HREF"><xsl:value-of select="/manpage/routine_idx"/></xsl:attribute>【同类函数列表】</A></p></xsl:if>
    ...
    <table class="methodtbl" cellspacing="0" cellpadding="0">
        <THEAD class="head">
            <TD width="20%" class="cell">
                <xsl:if match="*">例 程</xsl:if>
            </TD>
            <TD width="20%" class="cell">
                <xsl:if match="*">需要的头文件</xsl:if>
            </TD>
            <xsl:if test="routine/optional_headers[.$ge$' ']">
            <TD width="20%" class="cell">
                <xsl:if match="*">可选头文件</xsl:if>
            </TD>
            </xsl:if>
            <TD class="cell">
                <xsl:if match="*">兼容性</xsl:if>
            </TD>
        </THEAD>
