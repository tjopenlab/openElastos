...
<xsl:template match="domains">
    <xsl:if test="*"><h4>域</h4></xsl:if>
    <table class="methodtbl" cellspacing="0" cellpadding="0">
        <THEAD class="head">
            <TD width="37%" class="cell"><xsl:value-of select="/manpage/interface"/><xsl:if test="*">域</xsl:if></TD>
            <TD class="cell"><xsl:if test="*">描  述</xsl:if></TD>
        </THEAD>
...
<xsl:template match="attributes">
    <xsl:if test="*"><h4>属  性</h4></xsl:if>
    <table class="methodtbl" cellspacing="0" cellpadding="0">
        <THEAD class="head">
            <TD width="37%" class="cell"><xsl:value-of select="/manpage/interface"/><xsl:if test="*">属  性</xsl:if></TD>
            <TD class="cell"><xsl:if test="*">描  述</xsl:if></TD>
        </THEAD>
...
<xsl:template match="parent">
    <table class="methodtbl" cellspacing="0" cellpadding="0">
        <THEAD class="head">
            <TD width="37%" class="cell"><A class="method"><xsl:attribute name="href"><xsl:value-of select="interface_url"/>.htm</xsl:attribute><xsl:value-of select="interface_name"/></A><xsl:if test="*">方  法</xsl:if></TD>
            <TD class="cell"><xsl:if test="*">描  述</xsl:if></TD>
        </THEAD>
...
<xsl:template match="self">
    <table class="methodtbl" cellspacing="0" cellpadding="0">
        <THEAD class="head">
            <TD width="37%" class="cell"><xsl:value-of select="/manpage/interface"/><xsl:if test="*">方  法</xsl:if></TD>
            <TD class="cell"><xsl:if test="*">描  述</xsl:if></TD>
        </THEAD>
        <xsl:apply-templates select="method"/>
    </table>
...
<xsl:template match="macros">
    <xsl:if test="*"><h4>宏</h4></xsl:if>
    <table class="methodtbl" cellspacing="0" cellpadding="0">
        <THEAD class="head">
            <TD width="37%" class="cell"><xsl:value-of select="/manpage/interface"/><xsl:if test="*">宏 </xsl:if></TD>
            <TD class="cell"><xsl:if test="*">描  述</xsl:if></TD>
        </THEAD>
...
<xsl:template match="constructs">
    <xsl:if test="*"><h4>构造函数</h4></xsl:if>
    <table class="methodtbl" cellspacing="0" cellpadding="0">
        <THEAD class="head">
            <TD width="37%" class="cell"><xsl:value-of select="/manpage/interface"/><xsl:if test="*">构造函数</xsl:if></TD>
            <TD class="cell"><xsl:if test="*">描  述</xsl:if></TD>
        </THEAD>
...
<xsl:template match="operators">
    <xsl:if test="*"><h4>操作符</h4></xsl:if>
    <table class="methodtbl" cellspacing="0" cellpadding="0">
        <THEAD class="head">
            <TD width="37%" class="cell"><xsl:value-of select="/manpage/interface"/><xsl:if test="*">操作符</xsl:if></TD>
            <TD class="cell"><xsl:if test="*">描  述</xsl:if></TD>
        </THEAD>
...
<xsl:template match="callbacks">
    <xsl:if test="*"><h4>回 调 事 件</h4></xsl:if>
    <table class="methodtbl" cellspacing="0" cellpadding="0">
        <THEAD class="head">
            <TD  width="37%" class="cell"><xsl:value-of select="/manpage/interface"/><xsl:if test="*">回调事件</xsl:if></TD>
            <TD class="cell"><xsl:if test="*">描  述</xsl:if></TD>
        </THEAD>
