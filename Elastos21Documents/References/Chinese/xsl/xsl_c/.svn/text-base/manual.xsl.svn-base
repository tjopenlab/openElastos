...
<!--===========================  Chinese Edition Below  =================================-->
    ...
    <xsl:if test="/manpage/routine_idx[.!='']"><p style="font-size:90%;font-weight:bold;margin-left:0"><A class="method"><xsl:attribute name="HREF"><xsl:value-of select="/manpage/routine_idx"/></xsl:attribute>【同类函数列表】</A></p></xsl:if>
        ...
        <TABLE cellspacing="3" cellpadding="2" style="font-size:90%">
            <TR valign="top">
                <TD colspan="5" class="datatype"><xsl:value-of select="return_val"/>
                    <xsl:choose>
                        <xsl:when test="func_name[.!='']"><xsl:value-of select="func_name"/></xsl:when>
                    </xsl:choose>
                    (
                 </TD>
            </TR>
    ...
    <table class="methodtbl" cellspacing="0" cellpadding="0">
        <THEAD class="head">
            <TD width="20%" class="cell">
                <xsl:if test="*">例 程</xsl:if>
            </TD>
            <TD width="20%" class="cell">
                <xsl:if test="*">需要的头文件</xsl:if>
            </TD>
            <xsl:if test="routine/optional_headers[.!='']">
            <TD width="20%" class="cell">
                <xsl:if test="*">可选头文件</xsl:if>
            </TD>
            </xsl:if>
            <TD class="cell">
                <xsl:if test="*">兼容性</xsl:if>
            </TD>
        </THEAD>
