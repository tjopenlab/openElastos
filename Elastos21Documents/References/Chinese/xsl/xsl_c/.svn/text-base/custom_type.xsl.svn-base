...
<BODY>
    ...
    <div>
    <H3><xsl:choose>
        <xsl:when test="manpage/struct[.!='']">结构体：<xsl:value-of select="manpage/types"/></xsl:when>
        <xsl:when test="manpage/enum[.!='']">枚举类型：<xsl:value-of select="manpage/types"/></xsl:when>
        <xsl:when test="manpage/other[.!='']"><xsl:value-of select="manpage/types"/></xsl:when>
    </xsl:choose></H3>
    <p></p>
    </div>
<!--===========================  Chinese Edition Below  =================================-->
    ...
    <xsl:apply-templates select="manpage/other"/>
...
<xsl:template match="enum">
    <xsl:if test="*"><h4>枚举成员</h4></xsl:if>
    <table class="methodtbl" cellspacing="0" cellpadding="0">
        <THEAD class="head">
            <TD width="37%" class="cell"><xsl:if test="*">枚举成员</xsl:if></TD>
            <TD class="cell"><xsl:if test="*">枚举值</xsl:if></TD>
...
<xsl:template match="enum_member">
    <TR>
        ...
        <TD class="cell"><xsl:apply-templates select="mem_description"/></TD>
    </TR>
</xsl:template>
