...
<xsl:template match="c_style|cpp_style">
    <xsl:attribute name="title">See the <xsl:choose><xsl:when match="c_style">C Style</xsl:when><xsl:when match="cpp_style">C++ style</xsl:when></xsl:choose> usages of <xsl:value-of select="/manpage/function"/>.</xsl:attribute><xsl:choose>
...
<xsl:template match="sample_link">
    <A class='smplink'><xsl:attribute name="href"><xsl:value-of select="@url"/></xsl:attribute>Sample program and guide</A>
</xsl:template>
