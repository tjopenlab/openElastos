    ...
    <div id="nsbanner">
        <TABLE width="100%" border="1" cellspacing="0" rules="none" cellpadding="0" frame="below">
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <TD align="left" width="33%">COMMANDS &amp; TOOLS</TD>
                </xsl:otherwise>
            </xsl:choose>
            </TR>
        </TABLE>
...
<xsl:template match="title">
    <H3><xsl:if test="/manpage/category[.$ge$' ']"><xsl:value-of select="/manpage/category"/>:</xsl:if><xsl:value-of select="."/></H3>
</xsl:template>
...
<xsl:template match="manpage/parameters">
    ...
    <TABLE class="methodtbl" cellspacing="0" cellpadding="0">
        <THEAD class="head">
            <TD width="30%" class="cell">Parameter</TD>
